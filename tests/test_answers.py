...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_answer_by_id(async_client, get_new_question, get_question_response):
    question = get_new_question
    response = await async_client.get(f"http://localhost:8000/answers/{question.answer[0].id}")
...
async def test_del_answer_by_id(async_client, get_new_question):
    ...
    assert response.status_code == 200
    assert response.json() == f"deleted answer with id {added_answer.json()["id"]}"
